...
int main(int argc, char* argv[])
{
	...
	if ( mode == 256 )
	{
		...
		logfile << "Контрольные тесты ГОСТ Р 34.10-2001, 256 бит.\n";
		...
		for (dpid = 0, testnum = 1; dpid < 4; dpid++)
			for (int i = 0; i < 2; i++)
			{
				...
				logfile << fname << " - " << dp[dpid].oid
						<< ", signature of 256-bit hash from "
						<< h_name[i] << endl;

				make_one_test ( fname, dpid, d, h );
				testnum++;
			}
		logfile.close();
		cout << "OK." << endl;
	...
	else if ( mode == 512 )
	{
		...
		logfile.open("pmkinfo.txt");
		logfile << "Контрольные тесты ГОСТ Р 34.10-2012, 512 бит.\n";
		logfile << "Сформированы с помощью программной модели криптоалгоритма.\n\n";
		logfile << "Формат:\n";
		logfile << "  OID - ASCIIZ строка;\n";
		logfile << "  64 байта - значение хеш-функции;\n";
		logfile << "  128 байта - открытый ключ подписи;\n";
		logfile << "  128 байта - подпись.\n\n";
		...
		for (dpid = 6, testnum = 1; dpid < 9; dpid++)
			for (int i = 2; i < 4; i++)
			{
				...
				logfile << fname << " - " << dp[dpid].oid
						<< ", signature of 512-bit hash from "
						<< h_name[i] << endl;
